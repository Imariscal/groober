...
import { MdClose, MdWarningAmber, MdMedication } from 'react-icons/md';
import { Prescription } from '@/types/ehr';
import toast from 'react-hot-toast';

interface DeletePrescriptionConfirmationProps {
  isOpen: boolean;
  prescription: Prescription | null;
  onClose: () => void;
  onConfirm: (prescriptionId: string) => Promise<void>;
  loading?: boolean;
}

export function DeletePrescriptionConfirmation({
  isOpen,
  prescription,
  onClose,
  onConfirm,
  loading: externalLoading = false,
}: DeletePrescriptionConfirmationProps) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setLoading(true);
      await onConfirm(prescription.id);
      onClose();
          ...
          <div className="sticky top-0 z-20 bg-gradient-to-r from-red-600 via-red-600 to-red-700 px-6 py-4 flex items-center justify-between border-b border-red-600 shadow-sm rounded-t-xl">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-1">
                ...
                <h2 className="text-2xl font-bold text-white">Eliminar Prescripción</h2>
              </div>
          ...
          <div className="p-6">
            <div className="flex gap-4 mb-4">
              <div className="flex-shrink-0 flex items-center justify-center w-12 h-12 rounded-lg bg-red-100">
                <MdMedication className="w-6 h-6 text-red-600" />
              </div>
