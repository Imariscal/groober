...
import { veterinariansApi, VeterinarianUnavailablePeriod, VeterinarianCapacity, VeterinarianAvailability } from '@/api/veterinarians-api';
...
const VETERINARIAN_SPECIALTIES = [
...
const getSpecialtyLabel = (specialty: string): string => {
  return VETERINARIAN_SPECIALTIES.find(s => s.value === specialty)?.label || specialty;
};

type VeterinarianTab = 'availabilities' | 'unavailable' | 'capacities';

interface LocalVeterinarian {
  id: string;
  display_name: string;
  specialty?: string;
...
export const VeterinarianAvailabilityTab: React.FC = () => {
  const { user } = useAuth();
  const clinicId = user?.clinic_id || '';
  const clinicTimezone = useClinicTimezone();

  const [activeTab, setActiveTab] = useState<VeterinarianTab>('availabilities');
  const [veterinarians, setVeterinarians] = useState<LocalVeterinarian[]>([]);
  ...
  const [tempLicenseNumber, setTempLicenseNumber] = useState<string>('');

  const [availabilities, setAvailabilities] = useState<VeterinarianAvailability[]>([]);
  const [unavailablePeriods, setUnavailablePeriods] = useState<VeterinarianUnavailablePeriod[]>([]);
  const [capacities, setCapacities] = useState<VeterinarianCapacity[]>([]);
  ...
  useEffect(() => {
    const loadVeterinarians = async () => {
      try {
        setIsLoading(true);
        const data = await veterinariansApi.listVeterinarians(clinicId);
        // Map API response to LocalVeterinarian interface
        const mappedVeterinarians = data.map((vet: any) => ({
          id: vet.id,
          display_name: vet.displayName || vet.user?.name || 'Unknown',
          specialty: vet.specialty,
          license_number: vet.licenseNumber,
        }));
        setVeterinarians(mappedVeterinarians);
        if (mappedVeterinarians.length > 0) {
          const firstVet = mappedVeterinarians[0];
          setSelectedVeterinarianId(firstVet.id);
          setSelectedSpecialty(firstVet.specialty || '');
          setTempSpecialty(firstVet.specialty || '');
          setSelectedLicenseNumber(firstVet.license_number || '');
          setTempLicenseNumber(firstVet.license_number || '');
        }
      } catch (error) {
  ...
  // Load data when veterinarian or tab changes
  useEffect(() => {
    if (selectedVeterinarianId) {
      loadData();
    }
  }, [selectedVeterinarianId, activeTab]);

  const loadData = async () => {
    try {
      setIsLoading(true);
      if (activeTab === 'availabilities') {
        const data = await veterinariansApi.listAvailabilities(clinicId, selectedVeterinarianId);
        setAvailabilities(data);
      } else if (activeTab === 'unavailable') {
        const data = await veterinariansApi.listUnavailablePeriods(clinicId, selectedVeterinarianId);
        setUnavailablePeriods(data);
      } else if (activeTab === 'capacities') {
        const data = await veterinariansApi.listCapacities(clinicId, selectedVeterinarianId);
        setCapacities(data);
      }
    } catch (error) {
  ...
  const handleAddAvailability = async () => {
    if (!selectedVeterinarianId) return;
    try {
      setIsSaving(true);
      await veterinariansApi.createAvailability(clinicId, selectedVeterinarianId, availabilityForm);
  ...
  const handleDeleteAvailability = async (availabilityId: string) => {
    ...
    try {
      setIsSaving(true);
      await veterinariansApi.deleteAvailability(clinicId, selectedVeterinarianId, availabilityId);
  ...
  const handleAddUnavailable = async () => {
    ...
    try {
      ...
      await veterinariansApi.createUnavailablePeriod(clinicId, selectedVeterinarianId, payload);
  ...
  const handleDeleteUnavailable = async (periodId: string) => {
    ...
    try {
      setIsSaving(true);
      await veterinariansApi.deleteUnavailablePeriod(clinicId, selectedVeterinarianId, periodId);
  ...
  const handleAddCapacity = async () => {
    ...
    try {
      setIsSaving(true);
      await veterinariansApi.createCapacity(clinicId, selectedVeterinarianId, capacityForm);
  ...
  const handleDeleteCapacity = async (capacityId: string) => {
    ...
    try {
      setIsSaving(true);
      await veterinariansApi.deleteCapacity(clinicId, selectedVeterinarianId, capacityId);
  ...
  const handleUpdateVeterinarianProfile = async () => {
    if (!selectedVeterinarianId) return;
    try {
      setIsSaving(true);
      await veterinariansApi.updateVeterinarian(clinicId, selectedVeterinarianId, {
        specialty: tempSpecialty,
        licenseNumber: tempLicenseNumber,
      });
      toast.success('Perfil del veterinario actualizado');
      setSelectedSpecialty(tempSpecialty);
      setSelectedLicenseNumber(tempLicenseNumber);
  ...
  const handleCancelEditProfile = () => {
    ...
    setTempLicenseNumber(selectedLicenseNumber);
  };

  const handleVeterinarianChange = (vetId: string) => {
    const vet = veterinarians.find(v => v.id === vetId);
    if (vet) {
      setSelectedVeterinarianId(vetId);
      setSelectedSpecialty(vet.specialty || '');
      setTempSpecialty(vet.specialty || '');
      setSelectedLicenseNumber(vet.license_number || '');
      setTempLicenseNumber(vet.license_number || '');
      // Reset forms when switching veterinarian
      setShowAddAvailability(false);
      setShowAddUnavailable(false);
      setShowAddCapacity(false);
      setIsEditingProfile(false);
  ...
  if (isLoading && veterinarians.length === 0) {
  ...
  if (veterinarians.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        <MdLocalHospital className="text-4xl text-gray-400" />
        <p className="text-gray-600">No hay veterinarios creados</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 bg-gradient-to-br from-slate-50 to-slate-100 p-6 rounded-2xl">
      {/* Veterinarian Selector - Modern Card */}
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
        ...
        <select
          ...
          className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent bg-white font-medium text-slate-700"
        >
          {veterinarians.map((vet) => (
            <option key={vet.id} value={vet.id}>
              {vet.display_name || 'Sin nombre'}
              {vet.specialty && ` (${getSpecialtyLabel(vet.specialty)})`}
            </option>
          ))}
        </select>
      </div>

      {/* Veterinarian Profile - Edit Card */}
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
        ...
        {!isEditingProfile ? (
          <div className="space-y-3">
            <div>
              <p className="text-xs text-slate-500 font-semibold">ESPECIALIDAD</p>
              <p className="text-slate-800 font-semibold mt-1">
                {selectedSpecialty ? getSpecialtyLabel(selectedSpecialty) : '—'}
              </p>
            </div>
            <div>
              <p className="text-xs text-slate-500 font-semibold">NÚM. DE LICENCIA</p>
              <p className="text-slate-800 font-semibold mt-1">
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                ...
                {VETERINARIAN_SPECIALTIES.map((spec) => (
            ...
            <div className="flex gap-3">
              <button
                onClick={handleUpdateVeterinarianProfile}
                disabled={isSaving}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                      ...
                      <button
                        onClick={async () => {
                          if (isAvailable && dayAvailability) {
                            ...
                            try {
                              setIsSaving(true);
                              await veterinariansApi.deleteAvailability(clinicId, selectedVeterinarianId, dayAvailability.id);
                              toast.success('Horario eliminado');
                              await loadData();
                            } catch (error) {
                              toast.error('Error al eliminar horario');
                            ...
                            try {
                              setIsSaving(true);
                              await veterinariansApi.createAvailability(clinicId, selectedVeterinarianId, {
                                day_of_week: value,
                                start_time: '09:00',
                                end_time: '17:00',
                              });
                          ...
                          <div className="flex-1">
                            ...
                            <select
                              ...
                              onChange={async (e) => {
                                try {
                                  setIsSaving(true);
                                  await veterinariansApi.updateAvailability(
                                    clinicId,
        ...
        {activeTab === 'capacities' && (
          <div className="p-6 space-y-6">
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 p-5 rounded-xl border-2 border-emerald-200 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 border border-emerald-300 rounded-lg focus:ring-2 focus:ring-emerald-500"
                    rows={2}
                    placeholder="Ej: Personal reducido, conferencia"
                  />
                </div>
            ...
            {capacities.length === 0 ? (
              <div className="text-center py-12">
                <MdLocalHospital className="text-4xl text-slate-300 mx-auto mb-3" />
